...
const ESP32_IP_ADDRESS = "http://192.168.254.13:80"; 
...
app.post('/order', (req, res) => {
  ...
  io.emit('newOrder', { tableNumber: parseInt(tableNumber, 10), status: 'pending' });
...
app.post('/dispatch', async (req, res) => {
  ...
  try {
    ...
    await axios.post(`${ESP32_IP_ADDRESS}/dispatch`, null, { params: { table: tableNumber } });
    io.emit('orderStatusChange', { tableNumber: parseInt(tableNumber, 10), status: 'waiting_for_food' });
...
app.post('/status/food-loaded', (req, res) => {
  ...
  io.emit('orderStatusChange', { tableNumber: parseInt(tableNumber, 10), status: 'delivering' });
